...
class HuggingfaceModel(WhiteBoxModelBase):
    ...
    def generate(self, messages, input_field_name='input_ids', clear_old_history=True, stop_words: Optional[List[str]] = ["# Query: ","objects = ","assistant"], **kwargs):
        ...
        # Remove stop words and content after them from the output
        if stop_words and len(stop_words) > 0:
    ...
    def format_instance(self, query, jailbreak_prompt, response):
        prompt = jailbreak_prompt.replace('{query}', query) # The reason for not using str.format method is that jailbreak_prompt might be any string that could contain {...}, and str.format doesn't support ignoring undefined keys
    ...
    @property
    #@functools.cache  # Cache results to avoid repeated computation. Doesn't affect subclass overrides.
